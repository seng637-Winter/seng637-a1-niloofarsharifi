...
In this Lab we will try to make the first bug report for the SUT, here being the ATM application. This bug report will consist on 2 aproaches or techniques.
    ...
    -Exploratory: Consist of testing the software as we see fit, this type of approach is usually taken when no documentation of a particular
    system exist and it does not apply only to software. For example its the old way to do things by the saying "what does this button do?"
    then we record if something unexpected happens. As the name implies its an exploration of some sort.
    
    -Manual Scripted: By far the easiest way to testing software once the script has been developed. This type of testing makes the inclussion of new test easier by providing a path already thought for the tester to follow.
